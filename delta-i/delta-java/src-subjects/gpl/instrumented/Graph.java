...
class Graph {
	...
	public EdgeIter getEdges() {
		...
		if (General.split(General.and(General.FALSE, edgesNotBuilt))) {
			edgesNotBuilt = General.FALSE;
			edges = new LinkedList<EdgeIfc>();
			...
			for (VertexIter vxiter = getVertices(); vxiter.hasNext();) {
				...
				for (k2 = 0; k2 < sizeAV; k2 = k2 + 1) {
					// G
					edges.add(new Edge(v, v.adjacentVertices.get(k2),
							v.weightsList.get(k2)));
					// GN
	...
	DeltaBoolean edgesNotBuilt = General.TRUE;
	...
	EdgeIfc addEdge(Vertex start, Vertex end, DeltaInt weight) {
		// G
	...
	public Graph strongComponents() {
		...
		sortVertices(new Comparator<Vertex>() {
		  public int compare(Vertex v1, Vertex v2) {
		    if (General.split(General.gt(v1.finishTime, v2.finishTime)))
		      return -1;

		    if (General.split(General.eq_int(v1.finishTime,v2.finishTime)))
		      return 0;
		    return 1;
		  }
		});
	...
	public Graph computeTranspose(Graph the_graph) {
		...
		for (VertexIter vxiter = getVertices(); vxiter.hasNext();) {
			...
			for (VertexIter neighbors = theVertex.getNeighbors(); neighbors
				...
				newEdge = newGraph.addEdge(newVertex, newAdjacent, General.ZERO);
	...
	public DeltaBoolean cycleCheck() {
	...
	public Graph kruskal() {
		...
		sortEdges(new Comparator<EdgeIfc>() {
			public int compare(EdgeIfc e1, EdgeIfc e2) {
				if (General.split(General.lt(e1.getWeight(),e2.getWeight())))
					return -1;
				if (General.split(General.eq_int(e1.getWeight(),e2.getWeight())))
					return 0;
				return 1;
			}
	...
	public Graph prim(Vertex r) {
		...
		for (VertexIter vxiter = getVertices(); vxiter.hasNext();) {
			x = vxiter.next();
			x.pred = null;
			x.key = General.constant(Integer.MAX_VALUE);
		}

		// 4. and 5.
		root.key = General.ZERO;
		root.pred = null;
		...
		for (VertexIter vxiter = getVertices(); vxiter.hasNext();) {
			x = vxiter.next();
			if (General.split(General.neq_int(x.key,General.ZERO))) // this means, if this is not the root
			{
				queue.add(x);
				indx.add(x.getName());
			}
		...
		while (queue.size() != 0) {
			...
			for (EdgeIter edgeiter = u.getEdges(); edgeiter.hasNext(); k++) {
				...
				// 9. Relax (u,v w)
				if (isNeighborInQueue && General.split(General.lt(wuv, v.key))) {
					v.key = wuv;
					v.pred = u.getName();
					...
					int position = Collections.binarySearch(queue, v,
							new Comparator<Vertex>() {
								public int compare(Vertex v1, Vertex v2) {
									if (General.split(General.lt(v1.key, v2.key)))
										return -1;
									if (General.split(General.eq_int(v1.key, v2.key)))
										return 0;
									return 1;
								}
							});
		...
		for (VertexIter vxiter = getVertices(); vxiter.hasNext();) {
			...
			// and ajusts the adorns based on the old edge
			EdgeIfc theNewEdge = newGraph.addEdge(theNewPred, theNewVertex, General.ZERO);
