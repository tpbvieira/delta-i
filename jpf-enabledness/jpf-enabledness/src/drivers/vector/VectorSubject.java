 ...
 * @since JDK1.0
 */
public class VectorSubject<E> extends java.util.AbstractList<E> implements List<E>,
RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	public VectorSubject(int initialCapacity, int capacityIncrement) {
		super();
	 ...
	 *                if the specified initial capacity is negative
	 */
	public VectorSubject(int initialCapacity) {
	 ...
	 * @since 1.2
	 */
	public VectorSubject(Collection<? extends E> c) {
		elementCount = c.size();
		// 10% for growth
	...
	public Enumeration<E> elements() {
		return new Enumeration<E>() {
			...
			public E nextElement() {
				synchronized (VectorSubject.this) {
	...
	public synchronized Object clone() {
		try {
			VectorSubject<E> v = (VectorSubject<E>) super.clone();
	...
	public boolean addPre(E o){		
		return elementData.length >= elementCount && elementCount >= 0;
	}

	public boolean addPre(int index, E element) {
		return insertElementAtPre(element, index);
	}

	public boolean addAllPre(Collection<? extends E> c) {		
		return elementData.length >= elementCount && elementCount >=0;
	}

	public boolean addAllPre(int index, Collection<? extends E> c) {		
		return elementData.length >= elementCount && elementCount >= 0 && index <= elementCount && index >=0;
	}

	public boolean addElementPre(E obj) {
		return addPre(obj);
	}

	public boolean capacityPre() {
		return elementData != null;
	}

	public boolean clearPre() {
		return removeAllElementsPre();
	}

	public boolean containsPre(Object elem) {
		return elementData != null;
	}

	public boolean containsAllPre(Collection<?> c) {
		return elementData != null;
	}

	public boolean copyIntoPre(Object[] anArray) {
		return elementData.length >= elementCount && elementCount >= 0;
	}

	public boolean elementAtPre(int index) {
		return elementData.length >= elementCount && elementCount > 0 && index <= elementCount && index >=0;
	}

	public boolean elementsPre() {
		return elementData.length >= elementCount;
	}

	public boolean ensureCapacityPre(int minCapacity) {
		return minCapacity <= elementData.length && minCapacity >= 0;
	}

	public boolean firstElementPre() {		
		return elementData.length > 0 && elementData.length >= elementCount && elementCount > 0;
	}

	public boolean getPre(int index) {
		return elementData.length > 0 && elementCount > 0 && index <= elementCount && index >=0 && elementData.length >= elementCount;
	}

	public boolean indexOfPre(Object elem) {
		return indexOfPre(elem, 0);
	}

	public boolean indexOfPre(Object elem, int index) {
		return elementData.length > 0 && elementCount > 0 && index <= elementCount && index >=0 && elementData.length >= elementCount;
	}

	public boolean insertElementAtPre(E obj, int index) {
		return elementCount > 0 && index >= 0 && index <= elementCount && elementData.length >= elementCount;
	}
	...
	public boolean lastElementPre() {
		return elementData.length >= elementCount && elementCount > 0 && elementData.length > 0;
	}

	public boolean lastIndexOfPre(Object elem) {
		return elementData.length >= elementCount && elementCount > 0 && elementData.length > 0;
	}

	public boolean lastIndexOfPre(Object elem, int index) {		
		return elementData.length > 0 && elementData.length >= elementCount && elementCount > 0 && index <= elementCount && index >=0;
	}

	public boolean removePre(int index) {
		return elementData.length > 0 && elementData.length >= elementCount && elementCount > 0 && index <= elementCount && index >=0;
	}

	public boolean removePre(Object o) {
		return removeElementPre(o);
	}

	public boolean removeAllPre(Collection<?> c) {
		return elementData.length > 0 && elementData.length >= elementCount && elementCount > 0;
	}

	public boolean removeAllElementsPre() {
		return elementData.length > 0 && elementData.length >= elementCount && elementCount > 0;
	}

	public boolean removeElementPre(Object obj) {
		return elementData.length > 0 && elementCount > 0 && (indexOf(obj) >= 0);
	}

	public boolean removeElementAtPre(int index) {
		return elementData.length > 0 && elementCount > 0 && index <= elementCount && index >=0 && elementData.length >= elementCount;
	}

	public boolean retainAllPre(Collection<?> c) {		
		return true;
	}

	public boolean setPre(int index, E element) {
		return index <= elementCount && index >=0;
	}

	public boolean setElementAtPre(E obj, int index) {		
		return index <= elementCount && index >=0;
	}

	public boolean setSizePre(int newSize) {
		return newSize >= 0 && elementCount >=0;
	}

	public boolean sizePre() {
		return elementCount >= 0;
	}

	public boolean toArrayPre() {
		return elementCount <= elementData.length && elementCount >= 0;
	}

	public boolean toStringPre() {		
		return true;
	}

	public boolean trimToSizePre() {		
